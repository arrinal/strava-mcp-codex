...
import { errorResponse, parseSseOrJson } from './protocol.js';
...
export class RemoteMcpBridge {
    ...
    async post(payload, retry = true) {
        ...
        if (!response.ok)
            throw new Error(`upstream HTTP ${response.status}: ${redactString(body.slice(0, 500))}`);
...
export async function runStdioBridge() {
    ...
    for await (const raw of rl) {
        ...
        try {
            ...
            if (isRequest)
                writeJsonLine(result ?? { jsonrpc: '2.0', id: msg.id, result: {} });
        }
        catch (error) {
            console.error('bridge error:', redactString(error.message));
            const id = msg?.id;
            if (id !== undefined && id !== null)
                writeJsonLine(errorResponse(id, error.message));
        }
    }
}
