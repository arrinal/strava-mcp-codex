...
import { codexStatus, installCodexConfig, uninstallCodexConfig } from './codex.js';
...
export async function runCli(argv) {
    ...
    codex.command('install')
        ...
        const result = await installCodexConfig({ dryRun: Boolean(opts.dryRun), mode: mode });
        if (opts.dryRun)
            printJson({ ok: true, dryRun: true, configPath: result.configPath, changed: result.changed, server: mode === 'npx' ? { command: 'npx', args: ['-y', 'strava-mcp-codex'] } : { command: 'strava-mcp-codex' } });
        else
            printJson({ ok: true, configPath: result.configPath, changed: result.changed });
    });
    ...
    codex.command('uninstall')
        ...
        if (opts.dryRun)
            printJson({ ok: true, dryRun: true, configPath: result.configPath, changed: result.changed, action: 'remove mcp_servers.strava' });
        else
            printJson({ ok: true, configPath: result.configPath, changed: result.changed });
    });
